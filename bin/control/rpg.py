...
def save_result():
    print(
        f"Отработано {session['count_up'] + session['count_down']} из {session['all_found_groups']}. "
...
def rpg():
    ...
    for sample in reklama_posts:
        if (
            sample["views"]["count"] < sample_spam_post["views"]["count"]
            or "copy_history" in sample
            or sample["id"] in session["work"][session["name_session"]]["lip"]
            or (session["zagolovok"]["sosed"] or session["heshteg"]["reklama"]) in sample["text"]
    ...
    for key in session["rpg_words"].keys():
        new_groups = session["tools"].get_all(
            method="groups.search",
            max_count=1000,
            limit=session["group_count_max"],
            values={"q": key, "type": "group"},
        )["items"]
