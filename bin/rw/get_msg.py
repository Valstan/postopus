...
def get_msg(group, offset=0, count=1):
    ...
    try:
        ...
        if (
            "invalid access_token" in error_msg
            or "User authorization failed" in error_msg
            or "Access denied: this wall available only for community members" in error_msg
        ):
            pass
        else:
