...
def sort_po_foto(session, msg):
    ...
    if msg['attachments'][0]['type'] == 'photo' and\
            image_get(msg['attachments'][0]['photo']['sizes'][0]['url'], session['bases_path'] + 'image'):
        image = Image.open(session['bases_path'] + 'image')
        histo = image.histogram()
        hash_object = hashlib.md5(str(histo).encode())
        histo = hash_object.hexdigest()
        if histo in session[session['name_session']]['hash']:
            return session, []
        if sort_black_list(session['delete_msg_blacklist'], tesseract(session['bases_path'] + 'image')):
