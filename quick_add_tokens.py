...
# -*- coding: utf-8 -*-
"""
Быстрое добавление VK токенов через API
"""
import requests
import json
...
def add_token_via_api(token_data):
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"✅ Токен для региона {token_data['region']} добавлен успешно")
            return True
        else:
            print(f"❌ Ошибка при добавлении токена {token_data['region']}: {response.status_code}")
            print(f"   Ответ: {response.text}")
            return False
            
    except Exception as e:
        print(f"❌ Ошибка при добавлении токена {token_data['region']}: {e}")
        return False
...
def main():
    print("🚀 Быстрое добавление VK токенов в Postopus...")
    print("=" * 60)
    
    # Добавляем токены
    success_count = 0
    for token_data in TOKENS_TO_ADD:
        print(f"📝 Добавляем токен для региона: {token_data['region']}")
        if add_token_via_api(token_data):
            success_count += 1
        print()
    
    print("=" * 60)
    print(f"📊 Результат: {success_count}/{len(TOKENS_TO_ADD)} токенов добавлено")
