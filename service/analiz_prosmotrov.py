...
def analiz_prosmotrov(name_base):
    ...
    for sample in data:
        result += (
            f"{sample['name_group']} - {sample['len_posts'] // (sample['difference_second'] // 86400)} - "
    ...
    result += "\nСортировка по репостам:\n" "Имя группы * Репостов * Постов в день * Просмотров"
    for sample in data:
        result += (
            f"{sample['name_group']} - {sample['count_copy_history']} - "
